  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      if (writable == false)
        bid = inode_get_block_number (file_get_inode (file), tofs);
      struct page *p = NULL;
      p = page_file (upage, file, tofs, page_read_bytes, page_zero_bytes, writable, bid);
      if (p == NULL)
        return false;
...
setup_stack (void **esp, const char *file_args) 
{
  ...
  p = page_zero (((uint8_t *) PHYS_BASE) - PGSIZE, true); 
  if (p == NULL)
    return false;
  *esp = PHYS_BASE;
  page_in (p, false); 
